...
const { sugestMeBooks, sugestMeMovies } = require('./src/services');
...
app.post('/what-my-next', async (req, res) => {
    if (!req.body) {
...
app.post('/what-my-next-movie', async (req, res) => {
    if (!req.body) {
        return res.status(400).send('Livros não fornecidos.');
    }
    ...
    try {
        // Chamar a função generateText com o prompt fornecido
        const data = await sugestMeMovies(generativeAI, req.body);
